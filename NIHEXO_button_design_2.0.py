...
This code needs to run with updated Arduino code. Find this code at:
T:/RR/Projects/CP Exoskeleton/Exoskeleton Brace/Exo2.0 01.2018 in lab study__in development/Software Design/NIH_EXO_both_legs_testing_code_LUKE

Runs with Python 3.7.3 interpreter.

This GUI will receive and save data over wire or over bluetooth. It should operate at aroudn 20kHz during data
collection. The only irreplicable component is the Python Bluetooth library, 'Pybluez.' It's outdated and will only
run on Windows 10, with appropriate build tools. Good luck there...
...
called as an executable on the front end (i.e. it's called in this script).
...
Python:   Receiving:
          ...
          BEWARE .recv(). Set it beyond 1 byte, and you need a protocol that prefixes all communication from
          Arduino with a byte denoting communication length, in bytes. Don't mess with that unless you really need to.

LSL:      Expects an 8 channel signal when data is sent. Data to save is separated with '\t' by Arduino, so all
          receivedAndSaveData() methods parse with '\t' and push to LSL.

IMPORTANT INSTRUCTIONS:
Before Use:
1) Make sure all the libraries imported below are installed in your Python.
2) Reset Teensy so it starts up and looks for a settings string.

During Use:
4) Update and click 'Save' on LabRecorder11.b. Otherwise the data WILL NOT BE SAVED.
5) Check plots periodically.
...
from tkinter import *
# from Python_GUI_LUKE import ApplicationClass
...
# pybluez is only loaded if bluetooth is chosen

# if a library is misplaced (outside of your typical python library), you can use code like that below to import it
# import os.path
...
# import characters for interfacing with GUI
prompt_char = "^"
trial_start_char = "$"
trial_stop_char = "@"
end_string = '\n'
...
global control_type
global fsm_type
old_fsm_option = 0  # set this to whatever fsm_option starts on (2 states)
old_controller_option = 0
...
# These functions pull data from
...
def construct_test_param_string():  # pulls data from buttons and formats into proper string, separated with '/'
    """This method pulls data from page two (Testing Parameters) and packages them into a single string for
...
# info_RL_trig = StreamInfo('Trig_R', 'Exoskeleton', 8, 100, 'float32', 'JiComp')  # creates 8 channel LSL stream
# info_RL_angle = StreamInfo('Angle_R', 'Exoskeleton', 8, 100, 'float32', 'JiComp')  # creates 8 channel LSL stream
# info_RL_torque = StreamInfo('Torque_R', 'Exoskeleton', 8, 100, 'float32', 'JiComp')  # creates 8 channel LSL stream
# info_RL_fsr = StreamInfo('FSR_R', 'Exoskeleton', 8, 100, 'float32', 'JiComp')  # creates 8 channel LSL stream
# info_RL_current = StreamInfo('Current_R', 'Exoskeleton', 8, 100, 'float32', 'JiComp')  # creates 8 channel LSL stream
# info_RL_fsm = StreamInfo('FSM_R', 'Exoskeleton', 8, 100, 'float32', 'JiComp')  # creates 8 channel LSL stream
# info_RL_torque_set = StreamInfo('Torq_set_R', 'Exoskeleton', 8, 100, 'float32', 'JiComp')  # creates 8 channel LSL stream
# info_RL_pos_set = StreamInfo('Pos_set_R', 'Exoskeleton', 8, 100, 'float32', 'JiComp')  # creates 8 channel LSL stream
#
# outlet_RL_trig = StreamOutlet(info_RL_trig)
# outlet_RL_angle = StreamOutlet(info_RL_angle)
# outlet_RL_torque = StreamOutlet(info_RL_torque)
# outlet_RL_fsr = StreamOutlet(info_RL_fsr)
# outlet_RL_current = StreamOutlet(info_RL_current)
# outlet_RL_fsm = StreamOutlet(info_RL_fsm)
# outlet_RL_torque_set = StreamOutlet(info_RL_torque_set)
# outlet_RL_pos_set = StreamOutlet(info_RL_pos_set)
...
# == GUI LSL ==
#info_GUI = StreamInfo('GUI', 'Exoskeleton', 1)  # just for clocking GUI speed during testing
#outlet_GUI = StreamOutlet(info_GUI)
# info_LL_trig = StreamInfo('Trig_L', 'Exoskeleton', 8, 100, 'float32', 'JiComp')  # creates 8 channel LSL stream
# info_LL_angle = StreamInfo('Angle_L', 'Exoskeleton', 8, 100, 'float32', 'JiComp')  # creates 8 channel LSL stream
# info_LL_torque = StreamInfo('Torque_L', 'Exoskeleton', 8, 100, 'float32', 'JiComp')  # creates 8 channel LSL stream
# info_LL_fsr = StreamInfo('FSR_L', 'Exoskeleton', 8, 100, 'float32', 'JiComp')  # creates 8 channel LSL stream
# info_LL_current = StreamInfo('Current_L', 'Exoskeleton', 8, 100, 'float32', 'JiComp')  # creates 8 channel LSL stream
# info_LL_fsm = StreamInfo('FSM_L', 'Exoskeleton', 8, 100, 'float32', 'JiComp')  # creates 8 channel LSL stream
# info_LL_torque_set = StreamInfo('Torq_set_L', 'Exoskeleton', 8, 100, 'float32', 'JiComp')  # creates 8 channel LSL stream
# info_LL_pos_set = StreamInfo('Pos_set_L', 'Exoskeleton', 8, 100, 'float32', 'JiComp')  # creates 8 channel LSL stream
#
# outlet_LL_trig = StreamOutlet(info_LL_trig)
# outlet_LL_angle = StreamOutlet(info_LL_angle)
# outlet_LL_torque = StreamOutlet(info_LL_torque)
# outlet_LL_fsr = StreamOutlet(info_LL_fsr)
# outlet_LL_current = StreamOutlet(info_LL_current)
# outlet_LL_fsm = StreamOutlet(info_LL_fsm)
# outlet_LL_torque_set = StreamOutlet(info_LL_torque_set)
# outlet_LL_pos_set = StreamOutlet(info_LL_pos_set)
...
def start_trial():
    message = "py JC Click 'Start Trial' to begin. DATA WILL NOT PRINT DURING A TRIAL. Check the graphing application to monitor data. \n"
...
def receive_ble_data_and_send2LSL():
    ...
    while (L_state == 'rec' or R_state == 'rec') and buttons_state == 'on':
        ...
        # outlet_GUI.push_sample([counter])  # for clocking how fast the GUI goes
        # counter += 1
...
class MainView(tk.Frame):  # MainMenuPage,
    def __init__(self, *args, **kwargs):
        ...
        self.logoimage = PhotoImage(file="./graphics/nih_logo_3.gif")  # NIH logo
    ...
    def create_ble_window(self):
        ...
        # serverMACAddress1 = '00:06:66:FB:AD:2B'  # right leg
